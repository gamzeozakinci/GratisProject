...
| TC_019 | Add/Remove Product to Wishlist (Logged-In User) | `addAndRemoveProductFromWishlistWhenLoggedIn` | ❌ **Confirmed failing** by an actual `mvn test` run (twice). `PLPPage.addRWishlist1()`/`addRWishlist2()` both click `page.locator("span.hover\\:scale-110")` at a fixed index (`.first()` / `.nth(2)`) — an unscoped, generic Tailwind hover-effect class, not something specific to one product's wishlist heart. The "add" toggle passes reliably; the "remove" toggle never produces its confirmation toast. Most likely explanation: after the first click favorites a heart, that heart's own class list changes (filled-icon state), shifting which element the fixed index now resolves to — the second click probably favorites a *different*, still-unfavorited product instead of un-favoriting the first one. A real, currently-visible cookie-consent banner was also present in the failure screenshots and `acceptCookiesIfPresent()` was added in two places to rule it out — confirmed live that this did **not** fix the failure, so the indexed-locator theory above is the live lead, not yet fixed. |
...
| TC_021 | Remove Product from Wishlist (Logged-In User) | `removeProductFromWishlistWhenLoggedIn` | ⬜ Early stub. New `WishListPage.java` added, but its only method `wishFirstItem()` uses `page.locator(".absolute.top-2.right-2::nth2")` — `::nth2` isn't valid CSS syntax (not `:nth-child(2)`), so this locator won't resolve to anything. The test body has no assertion and ends mid-flow. |
...
**Progress: 12 / 30 implemented, 16 in progress, 1 confirmed failing, 1 not started.**
